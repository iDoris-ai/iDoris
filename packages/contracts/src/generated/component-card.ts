...
export const componentCardSchema = z.object({ "provider": z.object({ "id": z.string().min(1), "family": z.enum(["idoris","claude","openai","local","other"]), "tier": z.enum(["local","remote","lora"]), "capabilities": z.array(z.enum(["chat","reasoning","vision","asr","tts","coding","embedding","rerank"])).min(1), "privacy_class": z.enum(["local_only","any"]), "cost": z.object({ "input_per_m": z.number().gte(0), "output_per_m": z.number().gte(0) }).strict(), "locality": z.enum(["loopback","lan","remote"]) }).strict(), "form": z.enum(["http_service","spawn_cli","bundled_binary","nostr_node","mitm_proxy","batch_job"]), "endpoint": z.string().min(1), "version_pin": z.string().min(1), "privacy_class": z.enum(["local_only","any"]), "allowed_egress": z.array(z.enum(["none","loopback","lan","internet"])).min(1), "fallback_policy": z.enum(["fail_closed","next_in_chain"]), "fail_closed": z.boolean(), "load_policy": z.object({ "mode": z.enum(["resident","on_demand","evict_to_load"]), "keepalive": z.any().superRefine((x, ctx) => {
  ...
  }), "admission": z.enum(["coexist","requires_eviction"]) }).strict().optional() }).strict()
